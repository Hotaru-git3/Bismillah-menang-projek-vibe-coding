...
export default function DashboardCard({ expenses, monthlyBudget }: DashboardProps) {
  ...
  // We need dasharray formats. Circumference of a circle with r=15.9155 is 100.
  ...
  const offset1 = 25; // Start at top
  const offset2 = 100 - p1 + 25;
  const offset3 = 100 - p1 - p2 + 25;

  return (
    <div className="bg-white rounded-3xl shadow-[0_4px_24px_rgba(62,39,35,0.06)] p-6 mb-6 mx-4 relative overflow-hidden flex flex-col gap-6">
      <div className="flex justify-between items-start">
        <div>
          <p className="text-sm text-rk-brown/60 uppercase tracking-widest font-semibold mb-1">Bulan Ini</p>
          <h2 className="text-4xl font-serif text-rk-brown font-medium tracking-tight">
        ...
        <div className="relative w-24 h-24 shrink-0">
          <svg viewBox="0 0 36 36" className="w-full h-full transform">
            {/* Background ring */}
            <circle cx="18" cy="18" r="15.9155" fill="transparent" stroke="#f5e6d3" strokeWidth="4" />
            
            {/* Segments */}
            {p3 > 0 && (
              <motion.circle cx="18" cy="18" r="15.9155" fill="transparent" stroke="#8B3A3A" strokeWidth="4"
                strokeDasharray={dash3} strokeDashoffset={offset3}
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
              />
            )}
            {p2 > 0 && (
               <motion.circle cx="18" cy="18" r="15.9155" fill="transparent" stroke="#C8A45C" strokeWidth="4"
                 strokeDasharray={dash2} strokeDashoffset={offset2}
                 initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
               />
            )}
            {p1 > 0 && (
              <motion.circle cx="18" cy="18" r="15.9155" fill="transparent" stroke="#3E2723" strokeWidth="4"
          ...
          {/* Legend/Center text */}
          <div className="absolute inset-0 flex items-center justify-center flex-col text-[10px] leading-tight font-medium text-rk-brown">
             Sisa<br/>{runway} hr
          </div>
        </div>
      </div>
      ...
      <div className="mt-2">
        <div className="flex justify-between items-end mb-2">
           ...
           <span className="font-medium text-rk-brown">{Math.min(runway, daysInMonth)} / {daysInMonth} Hari</span>
        </div>
        <div className="w-full bg-rk-cream-dark h-2 rounded-full overflow-hidden border border-rk-brown/10">
           <motion.div 
             initial={{ width: 0 }}
             animate={{ width: `${Math.min(100, Math.max(0, (runway / daysInMonth) * 100))}%` }}
             transition={{ duration: 1, ease: 'easeOut' }}
             className="h-full bg-rk-gold-dark"
           />
        </div>
      </div>

      <div className="flex gap-4 border-t border-rk-brown/10 pt-4">
